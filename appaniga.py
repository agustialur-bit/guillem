...
def save_stats_jugador(match_id, data_consulta, df, teams, team_names):
    ...
    for jug in df["jugador"].unique():
        ...
        tl      = int(dj["accio"].str.contains("Tir lliure convertit",case=False,na=False).sum())
        ...
        impacte = pf-pc
        min_jug = dj["min_num"].max()-dj["min_num"].min()
    ...
    con.executemany("INSERT INTO stats_jugador (match_id,data_consulta,jugador,equip_nom,punts,cistelles_2,cistelles_3,tirs_lliures,faltes,accions,impacte,pts_per_min) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", rows)
    con.commit(); con.close()
...
with t5:
    ...
    if df_sj.empty:
        ...
        st.dataframe(df_rk[["Jugadora","Equip","Partits","Punts","Pts/p","C2","C3","TL","Faltes","Impacte","Imp/p"]],
            use_container_width=True,hide_index=True)
